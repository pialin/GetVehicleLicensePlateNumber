...
int main(int ArgumentCount, char** ArgumentVector)
{
	...
	for (size_t iImage = 0; iImage < ImagePathList.size(); iImage++)
	{
		...
		for (int iConvexY = 0; iConvexY < TitleRisingRow.size(); iConvexY++)
		{
			...
			ProjectYPassRect.width = GrayImageMat.cols;
			...
			Mat BinaryGrayTitleRow = GrayImageMat(Rect(0, TitleRisingRow[iConvexY], GrayImageMat.cols, TitleFallingRow[iConvexY] - TitleRisingRow[iConvexY]));
			threshold(
				GrayImageMat(Rect(0,TitleRisingRow[iConvexY],GrayImageMat.cols,TitleFallingRow[iConvexY]-TitleRisingRow[iConvexY])), //输入矩阵
				BinaryGrayTitleRow, //输出矩阵
				128, //迭代初始阈值
				255, //最大值（超过阈值将设为此值）
				CV_THRESH_BINARY_INV | CV_THRESH_OTSU //阈值化选择的方法:Otsu法
			);


			Mat DilatedGrayTitleRow;

			dilate(
				BinaryGrayTitleRow,//输入矩阵
				DilatedGrayTitleRow,//输出矩阵
				DilateElement,//膨胀结构元
				Point(-1, -1), //膨胀元锚点，在这里取中心作为锚点
				1 //进行膨胀的次数
			);
			...
			//创建X梯度X方向投影向量
			Mat  DilatedTitleRowProjectX(
				1,//矩阵行数
				DilatedGrayTitleRow.cols,//矩阵列数
				CV_8UC1,//矩阵值的类型（8位无符号整数单通道）
				Scalar(0)//矩阵填入的初始值
			);
			...
			for (int iCol = 0; iCol < DilatedGrayTitleRow.cols; iCol++)
			{
				SumTemp = 0;
				for (int iRow = 0; iRow < DilatedGrayTitleRow.rows; iRow++)
				{
					SumTemp += double(DilatedGrayTitleRow.at<uchar>(iRow, iCol));
				}
				DilatedTitleRowProjectX.at<uchar>(0, iCol) = unsigned char(SumTemp / DilatedGrayTitleRow.rows);
			}


			Mat BinaryTitleRowProjectX;
			threshold(
				DilatedTitleRowProjectX, //输入矩阵
				BinaryTitleRowProjectX, //输出矩阵
				128, //迭代初始阈值
				255, //最大值（超过阈值将设为此值）
				CV_THRESH_OTSU //阈值化选择的方法:Otsu法
			);

			Mat TitleRowProjectXStem(
				DilatedGrayTitleRow.rows,//矩阵行数
				DilatedGrayTitleRow.cols,//矩阵列数
				CV_8UC1,//矩阵值的类型（8位无符号整数单通道）
				Scalar(0)//矩阵填入的初始值
			...
			for (int iCol = 0; iCol < DilatedGrayTitleRow.cols; iCol++)
			{
				if (BinaryTitleRowProjectX.at<uchar>(0, iCol) == 0)
				{
					StemColor = 100;
				}
				else
				...
				for (int iRow = 0; iRow < DilatedTitleRowProjectX.at<uchar>(0, iCol) / 255.0*DilatedGrayTitleRow.rows; iRow++)
				{
					TitleRowProjectXStem.at<uchar>(iRow, iCol) = StemColor;
				}
			...
			BinaryTitleRowProjectX.at<uchar>(0, 0) = 0;
			BinaryTitleRowProjectX.at<uchar>(0, BinaryGrayTitleRow.cols - 1) = 0;
			for (int iCol = 0; iCol <BinaryTitleRowProjectX.cols; iCol++)
			{
				if (iCol < BinaryTitleRowProjectX.cols - 2 && BinaryTitleRowProjectX.at<uchar>(0, iCol) == 0 && BinaryTitleRowProjectX.at<uchar>(0, iCol + 1) == 255)
				{
					RisingCol.push_back(iCol);
				}
				else if (iCol < BinaryTitleRowProjectX.cols - 1 && BinaryTitleRowProjectX.at<uchar>(0, iCol) == 255 && BinaryTitleRowProjectX.at<uchar>(0, iCol + 1) == 0)
				{
			...
			for (int iConvexXCoupleRising = 0; iConvexXCoupleRising < RisingCol.size(); iConvexXCoupleRising++)
			{
				for (int iConvexXCoupleFalling = iConvexXCoupleRising; iConvexXCoupleFalling < FallingCol.size(); iConvexXCoupleFalling++)
				{
					...
					TempError = (FallingCol[iConvexXCoupleFalling] - RisingCol[iConvexXCoupleRising] - EstimatedTitleWidth) *
						...
						((FallingCol[iConvexXCoupleFalling] + RisingCol[iConvexXCoupleRising]) / 2.0 - TemplateTitleCenterX/TemplateWidth* DilatedGradX.cols) *
						((FallingCol[iConvexXCoupleFalling] + RisingCol[iConvexXCoupleRising]) / 2.0 - TemplateTitleCenterX /TemplateWidth* DilatedGradX.cols);
