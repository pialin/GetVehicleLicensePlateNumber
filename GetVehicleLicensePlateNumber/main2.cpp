...
const double TemplateTitleEnglishHeight = 8;
	...
	for (size_t iImage = 0; iImage < ImagePathList.size(); iImage++)
	{
		...
		for (int iConvexY = 1; iConvexY <= RisingRow.size(); iConvexY++)
		{
			...
			while (iConvexY < RisingRow.size() && RisingRow[iConvexY] - FallingRow[iConvexY - 1] <= TitleMaxGapHeight
				&& FallingRow[iConvexY] - RisingRow[iConvexY] <= TitleMaxEnglishHeight)
			{
				iConvexY++;
			}
		...
		for (int iConvexY = 0; iConvexY < TitleRisingRow.size(); iConvexY++)
		{
			...
			for (int iConvexX = 1; iConvexX <= RisingCol.size(); iConvexX++)
			{
				...
				while (iConvexX < RisingCol.size() && RisingCol[iConvexX] - FallingCol[iConvexX - 1] <= TitleMaxGapWidth)
				{
					iConvexX++;
				}
			...
			{
				Rect ProjectXPassRect;
				ProjectXPassRect.x = TitleRisingCol[iConvexX];
				ProjectXPassRect.y = 0;
				ProjectXPassRect.width = TitleFallingCol[iConvexX] - TitleRisingCol[iConvexX];
				ProjectXPassRect.height = DilatedGradX.rows;
				rectangle(GradXProjectXStem,//绘制矩形的对象
					ProjectXPassRect, //要绘制的矩形
					Scalar(255), //线条的颜色
					3,//线条宽度
					LINE_AA,//线型（抗混叠） 
					0 //??坐标数值（二进制）的小数位数
