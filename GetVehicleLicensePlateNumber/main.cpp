...
//bool CompareContourCenter(const vector<Point>  A, const vector<Point> B)
//{
//	double APointXSum = 0;
//	double APointYSum = 0;
//	double BPointXSum = 0;
//	double BPointYSum = 0;
//	for (size_t iPoint = 0; iPoint <A.size();iPoint++)
//	{
//		APointXSum = APointXSum + A[iPoint].x; 
//		APointYSum = APointYSum + A[iPoint].y; 
//	}
//	for (size_t iPoint = 0; iPoint <B.size();iPoint++)
//	{
//		BPointXSum = BPointXSum + B[iPoint].x; 
//		BPointYSum = BPointYSum + B[iPoint].y; 
//	}
//	double APointXMean = APointXSum/A.size() - CropRect.x;
//	double APointYMean = APointYSum/A.size() - CropRect.y;
//	double BPointXMean = BPointXSum/B.size() - CropRect.x;
//	double BPointYMean = BPointYSum/B.size() - CropRect.y;
//
//	bool CompareResult = (pow(APointXMean-TemplateTitlePlateMeanX,2.0) + pow(APointYMean-TemplateTitlePlateMeanY,2.0)) >
//		(pow(BPointXMean-TemplateTitlePlateMeanY,2.0) + pow(BPointYMean-TemplateTitlePlateMeanY,2.0));
...
int main(int ArgumentCount, char** ArgumentVector)
{
	...
	for (size_t iImage = 0;iImage<ImagePathList.size();iImage++)
	{
		...
		resize(RawImageMat,//输入图像
			...
			Size(0,0),//目标图像尺寸，设为0表示缩放比例由后面参数决定
			TemplateWidth/CropRect.width,//横向缩放系数
			TemplateWidth/CropRect.width,//纵向缩放系数
			CV_INTER_AREA//插值方式选择，CV_INTER_AREA为区域插值，在对图像进行缩小时比较合适
			);

		CropRect.x = int((TemplateWidth/CropRect.width*InputImageWidth - TemplateWidth)/2.0);
		CropRect.y = int((TemplateWidth/CropRect.width*InputImageHeight - TemplateHeight)/2.0);
		CropRect.width = int(TemplateWidth);
		CropRect.height = int(TemplateHeight);
		//计算输入图像的宽、高、面积和宽高比
		...
		for(size_t iRow =0;iRow<BlurGrayImageMat.rows;iRow++)
		{
			...
			{
				GrayImageGradX.at<uchar>(iRow,iCol) = abs(BlurGrayImageMat.at<uchar>(iRow,iCol-1) - BlurGrayImageMat.at<uchar>(iRow,iCol+1));
			}
		}
